...
namespace qnject {
    namespace helpers {
        ...
        inline decltype(auto) foldChildrenCastableBase(Fn fn, T init, QObjects objs) {
            for (QObject* o : objs) {
        ...
        inline decltype(auto) foldChildrenCastableTo(Fn fn, T init, QObject* m) {
            return foldChildrenCastableBase<ChildT>(fn, init, m->children());
        }

        template<typename ChildT, typename QObjects, typename Fn>
        inline nlohmann::json collectObjectsJson(QObjects&& objs, Fn fn) {
        ...
        inline nlohmann::json collectChildrenJson(QObject* m, Fn fn) {
    ...
    namespace helpers {
        ...
        inline void* stringToAddress(const std::string& addrStr) {
